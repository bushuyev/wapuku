...
impl Config {
    ...
    pub fn run(&self, module: &mut Module) -> Result<Option<ThreadCount>, Error> {
        ...
        let thread_count = inject_start(module, tls.as_ref(), &stack, thread_counter_addr, memory)?;
        ...
        inject_destroy(module, tls.as_ref(), &stack, memory)?;
...
fn find_tls(module: &mut Module) -> Result<Option<Tls>, Error> {
    if !has_export(module, "__wasm_init_tls") {
        return Ok(None);
    }
...
fn inject_start(
    ...
    // Newer rustc outputs may omit TLS init symbols entirely when no TLS
    // initialization is required for the module.
    if let Some(tls) = tls {
